...
class DocWriterAgent(BaseAgent):
    ...
    async def handle(self, msg: Message) -> None:
        ...
        self.log(f"dossier written for '{ir.name}'", msg=msg)
        await self.emit(
            Topic.DOCS_READY,
        ...
        return (
            "\n".join(
                [
                    ...
                    "## 3. MetaTrader 5",
                    "",
                    "MQL5 export is generated by the `mt5_deploy` agent on request — ask "
                    'KRISH to "automate this on MT5" and it will compile the EA from this '
                    "same IR, install it on the bridge, and start it on **demo** with a "
                    "live-vs-backtest parity check before anything touches a live account.",
                    "",
...
class PackagerAgent(BaseAgent):
    ...
    subscribes = (Topic.DOCS_READY, Topic.PACKAGE_REQUEST)
